...
class ShippingAddressViewController: UIViewController, LabelBootstrap {
    ...
    @IBAction func addShippingAddress(_ sender: UIButton) {
        ...
        let postcodeRegex:Regex! = labelRegex().getRegexForCountry(country: self.selectedCountry)
        
        if !postcodeRegex.matches(tfPostcode.text ?? "") {
        ...
        if switchAddress.isOn {
            let data = try? JSONEncoder().encode(self.oShippingAddress)
