...
class GettingStartedViewController: AccountParentVC, LabelBootstrap {
    ...
    @IBAction func btnNext(_ sender: UIButton) {
        ...
        self.service.wpRegister(username: "label_" + self.loginBuild.firstName, email: self.loginBuild.email, firstName: self.loginBuild.firstName, lastName: self.loginBuild.lastName, password: self.loginBuild.password) { (user,status) in
            ...
            if user != nil {
                ...
                if status == "552" {
                    LabelAlerts().openMoreInfo(title: NSLocalizedString("KaE-27-TJR.text", comment: "Oops! (Text)"), desc: NSLocalizedString("This email is already in use..text", comment: "This email is already in use. (Text)"), vc: self)
                } else {
