...
extension StatusOrderViewController:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // LOADER
        let frame = CGRect(x: 0, y: 0, width: cell.viewContainerLoader.frame.width, height: cell.viewContainerLoader.frame.height)
        let activityLoader = NVActivityIndicatorView(frame: frame, type: .ballPulse, color: UIColor.lightGray, padding: 0)
        
        cell.viewContainerLoader.isHidden = false
        ...
        let productImages = self.oAwCore.sortImagePostions(images: oOrder.basket[indexPath.row].storeItem.image)
                if let mainImgSrc = productImages[0].src {
                    if mainImgSrc != "" {
                        cell.viewContainerLoader.isHidden = false
                        activityLoader.startAnimating()
                        self.oAwCore.downloadImage(url: mainImgSrc, completion: { productImage in
                            activityLoader.stopAnimating()
                            cell.viewContainerLoader.isHidden = true
                            cell.ivProd.image = productImage
                        })
                    }
                }
