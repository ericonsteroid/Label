...
class OrderConfirmationSetViewController: ParentLabelVC, LabelBootstrap {
    ...
    @IBAction func paymentMethodTapped(_ sender: UIButton) {
        switch sender.tag {
        case 1:
            PaymentMethodBorder(indx: 1)
            paymentMethod = "1"
            self.view.endEditing(true)
            break
        case 2:
            PaymentMethodBorder(indx: 2)
            paymentMethod = "2"
            self.view.endEditing(true)
            break
        case 3:
            PaymentMethodBorder(indx: 3)
            paymentMethod = "3"
            self.view.endEditing(true)
            break
        default:
    ...
    override func viewDidLoad() {
        ...
        self.basket = sDefaults().getUserBasket()
        self.updateOrder()
        self.UpdateUI()
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let data = sDefaults().pref.object(forKey: sDefaults().userAddress) as? Data {
            oShippingAddress = try? JSONDecoder().decode(labelShippingAddress.self, from: data)
    ...
    func getShipping(method:LabelShippingMethod) -> NSMutableDictionary {
        ...
        case "flat_rate":
            ...
            tmpDict = [
                ...
                "total":method.flatRateShipping.shippingTotal
            ]
    ...
    func createApplePayBtn() -> PKPaymentButton {
        ...
        let button:PKPaymentButton! = PKPaymentButton(paymentButtonType: labelCore().applePayButtonType, paymentButtonStyle: labelCore().applePayButtonStyle)
    ...
    func PaymentMethodBorder(indx:Int) {
        removePaymentBorder()
        if indx == 1 {
...
extension OrderConfirmationSetViewController: STPPaymentContextDelegate, STPAddCardViewControllerDelegate {
    ...
    func addCardViewController(_ addCardViewController: STPAddCardViewController, didCreateToken token: STPToken, completion: @escaping STPErrorBlock) {
        ...
        self.oAwCore.createStripeOrder(email: tfEmail.text ?? "", token: token.tokenId, amount: amount, description: descProds, completion: { response in
            ...
            if response == nil {
                completion(nil)
                addCardViewController.show(LabelAlerts().openWarningController(title: "Payment failed", desc: "Please check your details"), sender: self)
                LabelLog().output(log: "Stripe payment failed")
            } else {
                switch response! {
                case "205":
                    self.dismiss(animated: true, completion: nil)
...
extension OrderConfirmationSetViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func UpdateUI() {
        ...
        case "flat_rate":
            ...
            let basketShipping = Double(self.activeMethod?.flatRateShipping.shippingTotal ?? "0") ?? 0
        ...
        case "free_shipping":
            ...
            if (!taxJSON.isEmpty) {
                ...
                // WORKOUT TOTAL
                self.lblTotal.text = oAwCore.woBasketTotal(sBasket: self.basket)
            }
